...
namespace CoreData.Desktop.FileSystem.CoreDataFileSystem
{
    // https://github.com/chromium/vs-chromium/tree/master/src/Core/Files
    public static class PathHelpers
    {
        ...
        {
            var result = path.IndexOf(prefix, 0, path.Length, StringComparison.CurrentCulture);
            if (result < 0)
                return false;
    ...
    public struct SplitPath
    {
        ...
        {
            _root = !string.IsNullOrEmpty(root) ? root : throw new ArgumentNullException(nameof(root));
